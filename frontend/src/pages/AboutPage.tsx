  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        {icon ? <div className="text-slate-700/90">{icon}</div> : null}
      </div>
...
/* -----------------------------
   JourneyShowcase
------------------------------ */
type JourneyItem = { year: string; title: string; desc: string; tag?: string };

function JourneyShowcase({ items, defaultIndex = 0 }: { items: JourneyItem[]; defaultIndex?: number }) {
  const reduced = usePrefersReducedMotion();
  const safe = items?.length ? items : [{ year: "—", title: "—", desc: "—", tag: "—" }];

  const [idx, setIdx] = React.useState(() => Math.max(0, Math.min(safe.length - 1, defaultIndex)));
      ...
      <div className="mx-auto w-full max-w-[980px]">
        ...
        <div className="mt-8 grid items-start gap-8 lg:grid-cols-[1fr_220px]">
          ...
          <div className="px-2 sm:px-6">
            ...
            <div className={cn("transition", reduced ? "" : "will-change-transform")} style={{ transform: reduced ? undefined : "translateZ(0)" }}>
              <h3 className="text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">{active.title}</h3>

              <p className="mt-5 max-w-[68ch] text-base leading-relaxed text-slate-700 sm:text-lg">{active.desc}</p>
      ...
      <div className={cn("appsTrack", dir === "l" ? "appsLeft" : "appsRight")}>
        {[...items, ...items].map((img, idx) => (
          <div key={`${key}-${idx}`} className="appsTile" title={img.alt || "app"}>
            <img src={img.src} alt={img.alt || "app"} className="h-full w-full object-cover" draggable={false} loading="lazy" />
          </div>
        ))}
      </div>
    </div>
...
/* -----------------------------
   AboutPage (i18n-ready)
------------------------------ */

type StoryBlock = { no: string; title: string; body: string; tag: string; highlight: string };
type WhoWeAreItem = { title: string; desc: string };
type AwardItem = { year: string; title: string; org: string; note?: string };

export default function AboutPage() {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();

  // (keep) mouse spotlight hook instance (not required but you had it)
  const hero = useMouseSpotlight();

  const reduced = usePrefersReducedMotion();

  // Stats (numbers can stay hardcoded, labels i18n)
  ...
  // Typed helper: pull arrays/objects from i18n JSON
  const getObj = <T,>(key: string, fallback: T): T => {
    const v = t(key, { returnObjects: true }) as unknown as T;
    if (!v) return fallback;
    // when key missing, i18next may return the key string itself
    if (typeof v === "string") return fallback;
    return v;
  };

  const authorizedBrands = useMemo(() => getObj<string[]>("about.brands.authorized", []), [i18n.language]); // optional

  // Assets for the office wall remain local
  ...
  // Pull content from i18n
  const storyBlocks = useMemo(
    () => getObj<StoryBlock[]>("about.story.blocks", []),
    [i18n.language]
  );

  const mission = t("about.missionVision.mission.body");
  const vision = t("about.missionVision.vision.body");

  const whoWeAre = useMemo(() => {
    const items = getObj<WhoWeAreItem[]>("about.whoWeAre.cards", []);
    return items.map((x, idx) => ({
      icon:
        idx === 0 ? (
          <Compass className="h-6 w-6 text-emerald-700" />
        ) : idx === 1 ? (
          <Smile className="h-6 w-6 text-emerald-700" />
        ) : (
  ...
  const journey = useMemo(() => getObj<JourneyItem[]>("about.journey.items", []), [i18n.language]);
  const awards = useMemo(() => getObj<AwardItem[]>("about.awards.items", []), [i18n.language]);

  // Offices texts
  const appsTitle = t("about.offices.title");
  const appsDesc = t("about.offices.desc");

  // HERO typed title (kept)
  const [typedHeroTitle, setTypedHeroTitle] = useState(reduced ? t("about.hero.companyName") : "");
  useEffect(() => {
    if (reduced) return;
    const full = t("about.hero.companyName");
    setTypedHeroTitle("");
    let i = 0;
  ...
  /* -----------------------------
     Story Flow inline section (i18n)
  ------------------------------ */
  const storyFlow = useMemo(() => {
    // fallback to storyBlocks if you want consistency
    const flow = getObj<
      Array<{
        id: "01" | "02" | "03";
        title: string;
        meta: string;
        tag: { label: string; tone: "amber" | "pink" };
        body: string;
      }>
    >("about.story.flow.steps", [] as any);

    return flow;
  }, [i18n.language]);

  const storyFlowHeader = useMemo(
    () =>
      getObj<{
        kicker: string;
        titleLine1: string;
        titleLine2: string;
        subtitle: string;
      }>("about.story.flow.header", { kicker: "SHD STORY FLOW", titleLine1: "—", titleLine2: "", subtitle: "" }),
    [i18n.language]
  );

  // Story media assets (paths stay local)
  const STORY_MEDIA = useMemo(
    () => ({
      HERO_VIDEO: "/videos/shd-story-hero.mp4",
      IMG_STEP1_A: "/images/about/story/s1a.jpg",
      IMG_STEP1_B: "/images/about/story/s1b.jpg",
      IMG_STEP2_A: "/images/about/story/s2a.jpg",
      IMG_STEP2_B: "/images/about/story/s2b.jpg",
      IMG_WIDE_A: "/images/about/story/s3wide-a.jpg",
      IMG_WIDE_B: "/images/about/story/s3wide-b.jpg",
      IMG_TOWER_A: "/images/about/story/tower-a.jpg",
      IMG_TOWER_B: "/images/about/story/tower-b.jpg",
    }),
    []
  );

  const storyVideoOverlay = useMemo(
    () =>
      getObj<{
        pill: string;
        ...
        desc: string;
      }>("about.story.videoOverlay", {
        pill: "From 0→1 to 1→100",
        headlineLine1: "Build local. Scale fast.",
        headlineLine2: "Grow sustainable.",
        desc: "",
      }),
    [i18n.language]
  );

  const storyVideoStats = useMemo(
    () =>
      getObj<Array<{ label: string; value: string }>>("about.story.videoStats", [
      ...
      <div className="min-h-screen bg-white text-slate-900">
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="inline-flex items-center gap-2 rounded-full bg-white/80 px-4 py-2 text-xs font-extrabold text-slate-900 ring-1 ring-slate-900/10 shadow-sm backdrop-blur">
                        ...
                        {t("about.hero.badges.companyStory")}
                      </span>
                      <span className="inline-flex items-center gap-2 rounded-full bg-white/75 px-4 py-2 text-xs font-bold text-slate-800 ring-1 ring-slate-900/10 shadow-sm backdrop-blur">
                        ...
                        {t("about.hero.badges.globalLocalizationGrowth")}
                      </span>
                      <span className="inline-flex items-center gap-2 rounded-full bg-white/75 px-4 py-2 text-xs font-bold text-slate-800 ring-1 ring-slate-900/10 shadow-sm backdrop-blur">
                        <Truck className="h-4 w-4" />
                        {t("about.hero.badges.omoOperations")}
                      </span>
                    </div>

                    <h1 className="mt-6 text-4xl font-black tracking-tight text-[#4b2e1f] sm:text-6xl">
                      {typedHeroTitle || t("about.hero.companyName")}
                    ...
                    <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
                      ...
                      >
                        {t("about.hero.cta.viewJobs")} <ArrowRight className="h-4 w-4" />
                      </Link>
                      ...
                      >
                        {t("about.hero.cta.readStory")} <ChevronRight className="h-4 w-4" />
                      </button>
                    </div>
                    ...
                    <div className="mt-7 grid gap-3 sm:grid-cols-3">
                      {[
                        {
                          label: t("about.hero.stats.years.label"),
                          value: years,
                          suffix: t("about.hero.stats.years.suffix"),
                          icon: <Target className="h-4 w-4" />,
                        },
                        {
                          label: t("about.hero.stats.brands.label"),
                          value: `${brands}+`,
                          suffix: t("about.hero.stats.brands.suffix"),
                          icon: <Building2 className="h-4 w-4" />,
                        },
                        {
                          label: t("about.hero.stats.ka.label"),
                          value: `${kaStores}+`,
                          suffix: t("about.hero.stats.ka.suffix"),
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-700">
                      <span className="inline-flex items-center gap-1 rounded-full bg-white/70 px-3 py-1.5 ring-1 ring-slate-900/10 backdrop-blur">
                        ...
                        {t("about.hero.chips.warehousing")}
                      </span>
                      <span className="inline-flex items-center gap-1 rounded-full bg-white/70 px-3 py-1.5 ring-1 ring-slate-900/10 backdrop-blur">
                        <Truck className="h-3.5 w-3.5" />
                        {t("about.hero.chips.localLogistics")}
                      </span>
                      <span className="inline-flex items-center gap-1 rounded-full bg-white/70 px-3 py-1.5 ring-1 ring-slate-900/10 backdrop-blur">
                        <HeartHandshake className="h-3.5 w-3.5" />
                        {t("about.hero.chips.afterSales")}
                      </span>
                    </div>
                  </div>

                  {/* RIGHT (removed Global network) */}
        ...
        <BgSection id="story" bg="/images/about/story.jpg" className="py-2">
          <div className="mx-auto w-full max-w-[1180px] px-4 sm:px-6 lg:px-10 py-12">
            <SectionHeader
              kicker={t("about.story.header.kicker")}
              icon={<Compass className="h-4 w-4" />}
              title={t("about.story.header.title")}
              desc={t("about.story.header.desc")}
            />
            ...
            {(() => {
              ...
              const FlipImage = ({
                ...
                React.useEffect(() => {
                  const tmr = window.setInterval(() => setOnA((v) => !v), intervalMs);
              ...
              // pick flow steps (ensure 3)
              const f0 = storyFlow?.[0];
              const f1 = storyFlow?.[1];
              const f2 = storyFlow?.[2];
                  ...
                  <div ref={flowRef} className="lg:col-span-7 scroll-mt-24">
                    <div className="border border-slate-200 bg-white">
                      <div className="flex flex-col gap-4 border-b border-slate-200 p-4 sm:p-5 lg:flex-row lg:items-start lg:justify-between">
                        <div className="min-w-0">
                          <div className="inline-flex items-center gap-2 border border-slate-200 bg-white px-2.5 py-1 text-[12px] font-black text-slate-700">
                            ...
                            {storyFlowHeader.kicker}
                          </div>

                          <div className="mt-3 text-[22px] sm:text-[28px] font-black leading-[1.12] tracking-tight text-slate-950">
                            {storyFlowHeader.titleLine1}
                            <br className="hidden sm:block" />
                            {storyFlowHeader.titleLine2}
                          </div>

                          <div className="mt-2 text-[12px] font-semibold text-slate-500">{storyFlowHeader.subtitle}</div>
                        </div>
                      ...
                      <div className="p-4 sm:p-5">
                          ...
                          <div className="grid gap-4 sm:gap-5 lg:grid-cols-12">
                            <div className="lg:col-span-7">
                              <div className="flex items-start gap-3">
                                <StepNo>01</StepNo>
                                <div className="min-w-0">
                                  <div className="text-[16px] sm:text-[18px] font-black text-slate-950">{f0.title}</div>
                                  <div className="mt-0.5 text-[12px] font-semibold text-slate-500">{f0.meta}</div>
                                </div>
                                <div className="ml-auto">
                                  <Tag tone={f0.tag.tone}>{f0.tag.label}</Tag>
                                </div>
                              </div>
                            ...
                            <div className="lg:col-span-5">
                              <FlipImage
                                a={STORY_MEDIA.IMG_STEP1_A}
                                b={STORY_MEDIA.IMG_STEP1_B}
                                alt="Story step 01"
                        ...
                        {f1 ? (
                          <div className="mt-6 grid gap-4 sm:gap-5 lg:grid-cols-12">
                            <div className="lg:col-span-4">
                              <FlipImage
                                a={STORY_MEDIA.IMG_TOWER_A}
                                b={STORY_MEDIA.IMG_TOWER_B}
                            ...
                            <div className="lg:col-span-8">
                              <div className="flex items-start gap-3">
                                <StepNo>02</StepNo>
                                <div className="min-w-0">
                                  <div className="text-[16px] sm:text-[18px] font-black text-slate-950">{f1.title}</div>
                                  <div className="mt-0.5 text-[12px] font-semibold text-slate-500">{f1.meta}</div>
                                </div>
                                <div className="ml-auto">
                                  <Tag tone={f1.tag.tone}>{f1.tag.label}</Tag>
                                </div>
                              </div>
                        ...
                        <div className="mt-6">
                          <FlipImage
                            a={STORY_MEDIA.IMG_WIDE_A}
                            b={STORY_MEDIA.IMG_WIDE_B}
                        ...
                        {f2 ? (
                          <div className="mt-6 border-t border-slate-200 pt-5">
                            <div className="flex items-start gap-3">
                              <StepNo>03</StepNo>
                              <div className="min-w-0">
                                <div className="text-[16px] sm:text-[18px] font-black text-slate-950">{f2.title}</div>
                                <div className="mt-0.5 text-[12px] font-semibold text-slate-500">{f2.meta}</div>
                              </div>
                              <div className="ml-auto">
                                <Tag tone={f2.tag.tone}>{f2.tag.label}</Tag>
                              </div>
                            </div>
        ...
        <section className="bg-white py-14">
          <style>{`
            ...
            .node img{
              ...
              animation: faceSpin var(--face, 0s) linear infinite;
            }
          ...
          <div className="mx-auto w-full max-w-[1180px] px-4 sm:px-6 lg:px-10">
            <div className="grid items-center gap-10 lg:grid-cols-2">
              {/* LEFT */}
              <div>
                <div className="mv-kicker">{t("about.missionVision.kicker")}</div>

                <h2 className="mv-h mt-4">
                  {t("about.missionVision.headline.line1")}
                  <br className="hidden sm:block" />
                  {t("about.missionVision.headline.line2")}
                </h2>
                ...
                <div className="mt-8 space-y-8">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                        <div className="text-[11px] font-extrabold tracking-[0.22em] text-slate-900/70">
                          {t("about.missionVision.mission.kicker")}
                        </div>
                  ...
                  <div className="border-t border-slate-900/10 pt-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                        <div className="text-[11px] font-extrabold tracking-[0.22em] text-slate-900/70">
                          {t("about.missionVision.vision.kicker")}
                        </div>
                        <div className="mt-1 text-lg font-black text-slate-900">
              ...
              <div className="lg:justify-self-end">
                <div className="orbitWrap">
                  ...
                  <div className="orbitCap">{t("about.missionVision.orbitCaption")}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="mx-auto w-full max-w-[1180px] px-4 sm:px-6 lg:px-10 py-12">
              <div className="text-center">
                <div className="inline-flex items-center gap-2 text-[11px] font-black tracking-[0.22em] text-slate-900">
                  ...
                  <span>{t("about.whoWeAre.kicker")}</span>
                </div>
              ...
              <div className="mt-8 grid gap-4 md:grid-cols-3">
                {whoWeAre.map((x) => (
                  <div key={x.title} className="rounded-2xl bg-white/95 p-6">
                    <div className="flex items-start gap-4">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-900/5">{x.icon}</div>
        ...
        <BgSection bg="/images/about/journey.jpg" className="py-2">
          <div className="mx-auto w-full max-w-[1180px] px-4 sm:px-6 lg:px-10 py-12">
            <SectionHeader
              kicker={t("about.journey.header.kicker")}
              icon={<Rocket className="h-4 w-4" />}
              title={t("about.journey.header.title")}
              desc={t("about.journey.header.desc")}
              align="center"
            />
        ...
        <BgSection bg="/images/about/awards.jpg" className="py-2">
          <div className="mx-auto w-full max-w-[1180px] px-4 sm:px-6 lg:px-10 py-12">
            <SectionHeader
              kicker={t("about.awards.header.kicker")}
              icon={<Award className="h-4 w-4" />}
              title={t("about.awards.header.title")}
              desc={t("about.awards.header.desc")}
            />

            {(() => {
              const exts = ["jpg", "png", "webp", "gif"] as const;
              const pickExt = (_i: number) => exts[0];
              ...
              const VideoTile = ({ src, className, label }: { src: string; className: string; label?: string }) => (
              ...
              const ImgTile = ({ src, className, label }: { src: string; className: string; label?: string }) => (
                <Tile className={cn("min-h-[140px] sm:min-h-[170px]", className)}>
                  <img
                    src={src}
                    alt={label || "award"}
              ...
              const FlipTile = ({
                ...
                React.useEffect(() => {
                  const tmr = window.setInterval(() => setIdx((v) => (v === 0 ? 1 : 0)), intervalMs);
                  return () => window.clearInterval(tmr);
                }, [intervalMs]);
              ...
              const TextTile = ({
                ...
                <Tile className={cn("min-h-[140px] sm:min-h-[170px]", className)}>
                  <img src={bgSrc} alt="award-bg" className="absolute inset-0 h-full w-full object-cover" loading="lazy" draggable={false} />
                  ...
                  <div className="relative p-5 sm:p-6 text-white">
                    <div className="flex items-center justify-between gap-3">
                      <div className="inline-flex items-center gap-2 rounded-full bg-black/35 px-3 py-1 text-[11px] font-black">
                        ...
                        {t("about.awards.grid.statPill")}
                      </div>
                      <YearPill year={t("about.awards.grid.statYear")} tone="dark" />
                    </div>
                    ...
                    <div className="mt-2 text-sm font-black text-white/95">{t("about.awards.grid.statTitle")}</div>

                    <div className="mt-4 text-sm leading-relaxed text-white/85">{t("about.awards.grid.statDesc")}</div>
                  </div>
                </Tile>
              );

              // Unique image assignment (NO DUPES)
              const IMG_F1A = getMedia(1);
              const IMG_F1B = getMedia(2);
              const IMG_6 = getMedia(5);
              const IMG_7 = getMedia(6);
              const IMG_8 = getMedia(7);
              const IMG_T1 = getMedia(8);
              const IMG_F2A = getMedia(9);
              const IMG_F2B = getMedia(10);
              const IMG_D2 = getMedia(11);
              const IMG_D3 = getMedia(12);
              const IMG_D4 = getMedia(13);
              const IMG_MOBILE_ONLY = getMedia(0);

              return (
                <div className="mt-8">
                  <div className="grid grid-cols-12 gap-3">
                    <VideoTile className="col-span-12 sm:col-span-4" src={VIDEO_HIGHLIGHTS} label={t("about.awards.grid.labels.highlights")} />

                    <FlipTile
                      className="col-span-6 sm:col-span-4"
                      aSrc={IMG_F1A}
                      bSrc={IMG_F1B}
                      label={t("about.awards.grid.labels.moments")}
                      intervalMs={2600}
                    />
                    ...
                    <VideoTextTile
                      ...
                      year={t("about.awards.grid.feature.year")}
                      videoSrc={VIDEO_DREAME_2024}
                      a={{
                        title: t("about.awards.grid.feature.title"),
                        org: t("about.awards.grid.feature.org"),
                      }}
                    />
                    ...
                    <TextTile
                      ...
                      titleClassName="text-base sm:text-lg"
                      a={{
                        year: t("about.awards.grid.textTile.year"),
                        title: t("about.awards.grid.textTile.title"),
                        org: t("about.awards.grid.textTile.org"),
                      }}
                      bgSrc={IMG_T1}
                    />
                    ...
                    <FlipTile
                      ...
                      aSrc={IMG_F2A}
                      bSrc={IMG_F2B}
                      label={t("about.awards.grid.labels.trusted")}
                      intervalMs={3000}
                    />
                  ...
                  <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                    ...
                    >
                      {t("about.awards.actions.explore")} <ArrowRight className="h-4 w-4" />
                    </Link>
                    ...
                    >
                      {t("about.awards.actions.backToTop")} <ChevronRight className="h-4 w-4 rotate-[-90deg]" />
                    </button>
                  </div>
                </div>
              );
